...
#Warm up model
# rubikpic = np.load('./rubikpic_random.npy')
...
#cv2.resizeWindow(winname, 1280 ,650)
...
try:
  ...
  while True:
    if(cv2.waitKey(50) & Next.is_pressed):
      ...
      call("sudo poweroff", shell=True) #poweroff to fix
      break
...
trans = {'U':'U','U\'':'U\'','U2':'U2',
      ...
      'B':'R','B\'':'R\'','B2':'R2',
      'L':'B','L\'':'B\'','L2':'B2',
      'R':'F','R\'':'F\'','R2':'F2'} #Translate rubik_solver view to my back view
  ...
  if(state==0):
    ...
    while(True):
      ...
      if(cv2.waitKey(1) & Back.is_pressed):
        ...
        # td = np.array([td])
        color_front = [4,4,5,2,5,1,1,5,3,3,4,0,0,1,5,0,4,1,0,2,5,4,2,0]# should be []
        #color_front = [0,0,0,0,0,0,0,0,2,2,2,1,1,1,2,2,1,1,2,2,2,1,1,1]
        # for i in range(0,1): #Use models to predict
    ...
    while(True):
      ...
      if(cv2.waitKey(1) & Back.is_pressed):
        ...
        # td = preprocess_input(td)
        # td = np.array([td])
        color_back = [3,0,3,1,1,0,3,2,2,5,3,1,4,5,4,1,2,5,4,2,0,2,3,3]# should be []
        #color_back = [3,3,3,3,3,3,3,3,4,4,4,5,5,5,4,4,5,5,4,4,4,5,5,5]
        # for i in range(0,1): #Use models to predict
    ...
    if(cube==solved_state):
      ...
      try:
        ans = utils.solve(cube, 'Kociemba')
        all_back = []
        cur_front = np.copy(color_front)
        cur_back = np.copy(color_back)
        ans_real = []
        for c in ans:
          ans_real.append(trans[str(c)])
        all_back.append(color_back)
        for c in ans_real:
          cur_front, cur_back = nextview(cur_front, cur_back, str(c))
          all_back.append(cur_back)
        img = draw_rubik(color_back,front=False)
        ...
        img = np.copy(ori)
        cv2.putText(img,'There is something wrong!',(90,480),font,0.8,(20,20,128),2,cv2.LINE_AA)
        cv2.imshow(winname,img)
        while True:
          if(cv2.waitKey(25) & Back.is_pressed):
            state = 4
    ...
    step = 0
    last_step = len(ans) # not real last step
